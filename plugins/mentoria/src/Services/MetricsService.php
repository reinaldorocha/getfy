...
class MetricsService
{
    public function tenantSummary(int $tenantId): array
    {
        ...
        $seconds7d = (int) DB::table('mentoria_study_sessions')
            ->where('tenant_id', $tenantId)
            ->where('is_active', true)
            ->where('studied_at', '>=', now()->subDays(7))
            ->sum('seconds');

        $questions = DB::table('mentoria_question_logs')
            ->where('tenant_id', $tenantId)
            ->where('is_active', true)
        ...
        return [
            ...
            'contests' => DB::table('mentoria_contests')->where('tenant_id', $tenantId)->where('is_active', true)->count(),
            'edicts' => DB::table('mentoria_edicts')->where('tenant_id', $tenantId)->where('is_active', true)->count(),
            'questions' => DB::table('mentoria_question_bank')->where('tenant_id', $tenantId)->where('is_active', true)->count(),
            'flashcard_decks' => DB::table('mentoria_flashcard_decks')->where('tenant_id', $tenantId)->where('is_active', true)->count(),
            'materials' => DB::table('mentoria_support_materials')->where('tenant_id', $tenantId)->where('is_active', true)->count(),
    ...
    public function studentSummary(int $tenantId, int $studentId): array
    {
        ...
        $currentQuestions = $this->questionStats(
            $tenantId,
            $studentId,
            today()->subDays(6)->toDateString(),
            today()->toDateString(),
        );
        $previousQuestions = $this->questionStats(
            $tenantId,
            $studentId,
            today()->subDays(13)->toDateString(),
            today()->subDays(7)->toDateString(),
        );
        $edictPace = $this->edictPace($tenantId, $studentId);
        $lastStudy = DB::table('mentoria_study_sessions')
            ->where('tenant_id', $tenantId)
            ->where('student_id', $studentId)
            ->where('is_active', true)
            ->max('studied_at');

        $pendingReviews = DB::table('mentoria_scheduled_reviews')
            ->where('tenant_id', $tenantId)
            ->where('student_id', $studentId)
            ->where('is_active', true)
        ...
        return [
            ...
            'questions_7d' => $currentQuestions['solved'],
            'accuracy_7d' => $currentQuestions['accuracy'],
            'questions_previous_7d' => $previousQuestions['solved'],
            'accuracy_previous_7d' => $previousQuestions['accuracy'],
            'pending_reviews' => $pendingReviews,
            'flashcards_due' => DB::table('mentoria_flashcard_reviews')
                ->where('tenant_id', $tenantId)
                ->where('student_id', $studentId)
            ...
            'edict_pace_percentage' => $edictPace['pace_percentage'],
            'edict_target_percentage' => $edictPace['target_percentage'],
            'edict_name' => $edictPace['name'],
            'edict_exam_date' => $edictPace['exam_date'],
    ...
    public function summary(int $tenantId, int $studentId, ?string $contestId = null, ?string $start = null, ?string $end = null): array
    {
        ...
        $sessionsPeriod = DB::table('mentoria_study_sessions')
        ...
        $logsPeriod = DB::table('mentoria_question_logs')
            ->where('tenant_id', $tenantId)->where('student_id', $studentId)->where('is_active', true);
        ...
        $sessionsTotal = DB::table('mentoria_study_sessions')
            ->where('tenant_id', $tenantId)->where('student_id', $studentId)->where('is_active', true);
        $this->applyContest($sessionsTotal, $contestId);

        $logsTotal = DB::table('mentoria_question_logs')
            ->where('tenant_id', $tenantId)->where('student_id', $studentId)->where('is_active', true);
        ...
        $mockQuery = DB::table('mentoria_mock_exams')
        ...
        $todaySessions = DB::table('mentoria_study_sessions')
            ->where('tenant_id', $tenantId)->where('student_id', $studentId)->where('is_active', true)
            ->whereDate('studied_at', today());
        $this->applyContest($todaySessions, $contestId);

        $todayQuestions = DB::table('mentoria_question_logs')
            ->where('tenant_id', $tenantId)->where('student_id', $studentId)->where('is_active', true)
        ...
        return [
            ...
            'mocks_total' => DB::table('mentoria_mock_exams')
                ->where('tenant_id', $tenantId)->where('student_id', $studentId)
    ...
    public function subjects(int $tenantId, int $studentId, string $contestId, ?string $start = null, ?string $end = null): array
    {
        ...
        foreach ($subjects as $subject) {
            ...
            $mockStats = DB::table('mentoria_mock_exam_subject_results as r')
                ->join('mentoria_mock_exams as m', 'm.id', '=', 'r.mock_exam_id')
            ...
            $latestMock = DB::table('mentoria_mock_exam_subject_results as r')
                ->join('mentoria_mock_exams as m', 'm.id', '=', 'r.mock_exam_id')
                ->where('r.tenant_id', $tenantId)
                ->where('r.subject_id', $subject->id)
                ->where('r.is_active', true)
    ...
    private function coverage(int $tenantId, int $studentId, ?string $contestId): array
    {
        $edictIds = DB::table('mentoria_student_edicts as se')
            ->join('mentoria_edicts as e', 'e.id', '=', 'se.edict_id')
            ->where('se.tenant_id', $tenantId)->where('se.student_id', $studentId)
            ->where('se.is_active', true)->where('e.is_active', true)
            ->when($contestId, fn ($query) => $query->where('e.contest_id', $contestId))
            ->pluck('e.id');

        return $this->coverageForEdictIds($tenantId, $studentId, $edictIds);
    }

    private function coverageForEdictIds(int $tenantId, int $studentId, $edictIds): array
    {
        $edictIds = collect($edictIds)->filter()->values();
        if ($edictIds->isEmpty()) {
            return [0, 0, 0, 0];
        }
    ...
    private function questionStats(int $tenantId, int $studentId, string $start, string $end): array
    {
        $stats = DB::table('mentoria_question_logs')
            ->where('tenant_id', $tenantId)
            ->where('student_id', $studentId)
            ->where('is_active', true)
            ->whereDate('recorded_at', '>=', $start)
            ->whereDate('recorded_at', '<=', $end)
            ->selectRaw('COALESCE(SUM(solved),0) solved, COALESCE(SUM(correct),0) correct')
            ->first();
    ...
    private function edictPace(int $tenantId, int $studentId): array
    {
        $edicts = DB::table('mentoria_student_edicts as se')
            ->join('mentoria_edicts as e', 'e.id', '=', 'se.edict_id')
            ...
            ->whereNotNull('c.exam_date')
            ->select('e.id', 'e.name', 'c.exam_date', 'se.created_at as assigned_at')
            ->get();

        $mostDelayed = ['pace_percentage' => 0, 'target_percentage' => 0, 'name' => null, 'exam_date' => null, 'gap' => 0];
        foreach ($edicts as $edict) {
            [$covered, $total] = $this->coverageForEdictIds($tenantId, $studentId, [(string) $edict->id]);
            if ($total === 0) {
                continue;
            }

            $progress = round(($covered / $total) * 100, 1);
            $assignedAt = Carbon::parse($edict->assigned_at)->startOfDay();
            $examDate = Carbon::parse($edict->exam_date)->startOfDay();
            $duration = max(1, $assignedAt->diffInDays($examDate));
            $elapsed = min($duration, max(0, $assignedAt->diffInDays(today(), false)));
            $target = round(min(100, ($elapsed / $duration) * 100), 1);
            $gap = $target - $progress;

            if ($gap > $mostDelayed['gap']) {
                $mostDelayed = [
                    'pace_percentage' => $progress,
                    'target_percentage' => $target,
                    'name' => (string) $edict->name,
    ...
    private function subjectCoverage(int $tenantId, int $studentId, string $subjectId): array
    {
        ...
        foreach ($topics as $topicId) {
            $subIds = DB::table('mentoria_subtopics')
    ...
    {
        return (int) DB::table('mentoria_study_sessions')
            ->where('tenant_id', $tenantId)->where('student_id', $studentId)->where('is_active', true)
