...
class ContentAudienceService
{
    ...
    public function targets(int $tenantId, string $contentType, string $contentId): array
    {
        ...
        return DB::table('mentoria_content_targets')
            ->where('tenant_id', $tenantId)
            ->where('content_type', $contentType)
            ->where('content_id', $contentId)
            ->orderBy('target_type')
    ...
    public function visibleIds(User $student, int $tenantId, string $contentType): array
    {
        ...
        $rows = DB::table('mentoria_content_targets')
            ->where('tenant_id', $tenantId)
            ->where('content_type', $contentType)
    ...
    private function normalizeTargets(int $tenantId, array $targets): array
    {
        ...
        foreach ($targets as $target) {
            ...
            if (! in_array($type, self::TARGET_TYPES, true)) {
                throw ValidationException::withMessages(['targets' => 'Tipo de disponibilização Mentoria inválido.']);
            }
    ...
    {
        $exists = match ($type) {
            'product' => DB::table('mentoria_products')
                ->join('products', 'products.id', '=', 'mentoria_products.product_id')
                ->where('mentoria_products.tenant_id', $tenantId)
                ->where('mentoria_products.product_id', $id)
                ->where('mentoria_products.is_active', true)
                ->where('products.tenant_id', $tenantId)
                ->exists(),
            'contest' => DB::table('mentoria_contests')
                ->where('tenant_id', $tenantId)->where('id', $id)->where('is_active', true)->exists(),
            'edict' => DB::table('mentoria_edicts')
                ->where('tenant_id', $tenantId)->where('id', $id)->where('is_active', true)->exists(),
    ...
    private function studentContext(User $student, int $tenantId): array
    {
        ...
        $contests = DB::table('mentoria_student_contests')
            ->where('tenant_id', $tenantId)
        ...
        $edicts = DB::table('mentoria_student_edicts')
            ->where('tenant_id', $tenantId)
            ->where('student_id', $student->id)
            ->where('is_active', true)
            ->pluck('edict_id')
    ...
    private function assertContentType(string $contentType): void
    {
        if (! in_array($contentType, self::CONTENT_TYPES, true)) {
            throw ValidationException::withMessages(['content_type' => 'Tipo de conteúdo Mentoria inválido.']);
        }
    }
}
