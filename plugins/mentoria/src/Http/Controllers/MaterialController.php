...
class MaterialController extends Controller
{
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        DB::table('mentoria_support_materials')->insert([
            'id' => $id,
            'tenant_id' => $tenantId,
    ...
    public function update(Request $request, string $material): JsonResponse
    {
        ...
        if (! empty($data['edict_id'])) {
            $this->requireTenantRow('mentoria_edicts', $tenantId, $data['edict_id']);
        }
        ...
        DB::table('mentoria_support_materials')->where('tenant_id', $tenantId)->where('id', $material)->update($update);
    ...
    public function destroy(Request $request, string $material): JsonResponse
    {
        ...
        DB::transaction(function () use ($tenantId, $material): void {
            DB::table('mentoria_support_materials')
    ...
    public function updateStudentProgress(Request $request, int $tenant, string $material): JsonResponse
    {
        ...
        $existing = DB::table('mentoria_material_progress')
        ...
        if ($existing) {
            DB::table('mentoria_material_progress')->where('id', $existing->id)->update($payload);
            $id = $existing->id;
        } else {
            $id = (string) Str::uuid();
            DB::table('mentoria_material_progress')->insert($payload + [
                'id' => $id,
                'tenant_id' => $tenant,
    ...
    private function storeUploadedFile(Request $request, int $tenantId): array
    {
        ...
        $path = $storage->putFileAs('mentoria/'.$tenantId.'/materials', $file, $safeName);
    ...
    {
        $query = DB::table('mentoria_support_materials')
            ->where('tenant_id', $tenantId)
            ->where('id', $materialId);
