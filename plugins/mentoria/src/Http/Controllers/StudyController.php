...
class StudyController extends Controller
{
    ...
    public function studentCalendar(Request $request, int $tenant): JsonResponse
    {
        ...
        $query = DB::table('mentoria_schedule_items as i')
            ->join('mentoria_schedules as s', 's.id', '=', 'i.schedule_id')
    ...
    private function persistSchedule(Request $request, int $tenant, User $student, User $creator, ?array $generatedItems): JsonResponse
    {
        ...
        DB::transaction(function () use ($tenant, $student, $creator, $data, $items, $previous, $version, $scheduleId, $type): void {
            if ($previous) {
                DB::table('mentoria_schedules')->where('id', $previous->id)
                    ->update(['is_active' => false, 'state' => 'concluido', 'updated_at' => now()]);
            }

            DB::table('mentoria_schedules')->insert([
                'id' => $scheduleId,
                'tenant_id' => $tenant,
            ...
            foreach ($items as $index => $item) {
                ...
                DB::table('mentoria_schedule_items')->insert($this->scheduleItemPayload($tenant, $scheduleId, $item, $index + 1));
            }
        });

        $warning = $this->syncReviewSchedule($tenant, (int) $student->id, (string) $data['contest_id']);
    ...
    private function persistScheduleItem(Request $request, int $tenant, string $schedule): JsonResponse
    {
        ...
        $id = (string) Str::uuid();
        DB::table('mentoria_schedule_items')->insert($this->scheduleItemPayload($tenant, $schedule, $data, (int) ($data['position'] ?? 0), $id));
    ...
    private function applyItemUpdate(Request $request, int $tenant, int $studentId, object $row, bool $canPlan): JsonResponse
    {
        ...
        DB::transaction(function () use ($tenant, $studentId, $row, $data, $oldStatus, $newStatus, $createSession): void {
            ...
            $data['updated_at'] = now();
            DB::table('mentoria_schedule_items')->where('tenant_id', $tenant)->where('id', $row->id)->update($data);

            if ($createSession && $oldStatus !== 'concluido' && $newStatus === 'concluido') {
                $schedule = DB::table('mentoria_schedules')->where('id', $row->schedule_id)->first();
                $duration = (int) ($data['duration_minutes'] ?? $row->duration_minutes ?? 0);
                if ($duration > 0) {
                    DB::table('mentoria_study_sessions')->insert([
                        'id' => (string) Str::uuid(),
                        'tenant_id' => $tenant,
    ...
    private function deleteItemAndLinkedSession(int $tenant, int $studentId, string $itemId): void
    {
        DB::transaction(function () use ($tenant, $studentId, $itemId): void {
            DB::table('mentoria_study_sessions')
                ...
                ->update(['is_active' => false, 'updated_at' => now()]);
            DB::table('mentoria_schedule_items')->where('tenant_id', $tenant)->where('id', $itemId)->delete();
        });
    }
    ...
    {
        $row = DB::table('mentoria_study_sessions')
        ...
        DB::table('mentoria_study_sessions')->where('id', $session)->update($data);
    ...
    {
        $updated = DB::table('mentoria_study_sessions')
    ...
    {
        $row = DB::table('mentoria_question_logs')
        ...
        DB::table('mentoria_question_logs')->where('id', $log)->update($data);
    ...
    {
        $updated = DB::table('mentoria_question_logs')
    ...
    private function persistReview(Request $request, int $tenant, int $studentId): JsonResponse
    {
        ...
        if ($existing) {
            DB::table('mentoria_scheduled_reviews')->where('id', $existing->id)->update($payload + ['updated_at' => now()]);
        } else {
            DB::table('mentoria_scheduled_reviews')->insert($payload + [
                'id' => $id,
                'tenant_id' => $tenant,
                'student_id' => $studentId,
                'created_at' => now(),
        ...
        $warning = $this->syncReviewSchedule($tenant, $studentId, (string) $payload['contest_id']);

        return response()->json(['ok' => true, 'id' => $id, 'warning' => $warning], $existing ? 200 : 201);
    }

    private function updateReviewRow(Request $request, int $tenant, int $studentId, string $review): JsonResponse
    {
        $current = DB::table('mentoria_scheduled_reviews')
        ...
        DB::table('mentoria_scheduled_reviews')->where('id', $review)->update($update);

        $warning = $this->syncReviewSchedule($tenant, $studentId, (string) $merged['contest_id']);
        if ($current->contest_id && $current->contest_id !== $merged['contest_id']) {
            $warning ??= $this->syncReviewSchedule($tenant, $studentId, (string) $current->contest_id);
        }
    ...
    {
        $current = DB::table('mentoria_scheduled_reviews')
            ->where('tenant_id', $tenant)->where('student_id', $studentId)->where('id', $review)->first(['contest_id']);
        $updated = DB::table('mentoria_scheduled_reviews')
    ...
    private function syncReviewSchedule(int $tenant, int $studentId, string $contestId): ?string
    {
        if ($contestId === '') {
            return null;
        }

        $this->reviewSchedule->syncForContest($tenant, $studentId, $contestId);
        $schedule = $this->activeSchedule($tenant, $studentId, $contestId);
        if (! $schedule) {
            return null;
        }

        try {
            $this->engine->reprogram($tenant, $studentId, $schedule->id);
        } catch (\Throwable $e) {
            return $e->getMessage();
        }
    ...
    private function persistMockExam(Request $request, int $tenant, int $studentId): JsonResponse
    {
        ...
        DB::transaction(function () use ($tenant, $studentId, $id, $data): void {
            DB::table('mentoria_mock_exams')->insert($this->mockExamPayload($tenant, $studentId, $id, $data));
    ...
    {
        $current = DB::table('mentoria_mock_exams')
        ...
        DB::transaction(function () use ($tenant, $mock, $update, $data): void {
            DB::table('mentoria_mock_exams')->where('id', $mock)->update($update);
    ...
    {
        $updated = DB::table('mentoria_mock_exams')
        ...
        DB::table('mentoria_mock_exam_subject_results')
    ...
    private function persistExamConfiguration(Request $request, int $tenant, int $studentId, string $contestId, int $actorId): JsonResponse
    {
        ...
        $existing = DB::table('mentoria_exam_configurations')
            ->where('tenant_id', $tenant)
            ->where('student_id', $studentId)
        ...
        if ($existing) {
            DB::table('mentoria_exam_configurations')->where('id', $existing->id)->update($payload);
            $id = $existing->id;
        } else {
            $id = (string) Str::uuid();
            DB::table('mentoria_exam_configurations')->insert($payload + [
                'id' => $id,
                'tenant_id' => $tenant,
                'student_id' => $studentId,
    ...
    {
        $raw = DB::table('mentoria_exam_configurations')
            ->where('tenant_id', $tenant)
            ->where('student_id', $studentId)
            ->where('contest_id', $contestId)
            ->value('configuration');
    ...
    private function persistNotebook(Request $request, int $tenant, int $studentId): JsonResponse
    {
        ...
        $id = (string) Str::uuid();
        DB::table('mentoria_notebooks')->insert([
            'id' => $id,
            'tenant_id' => $tenant,
            'student_id' => $studentId,
    ...
    {
        $exists = DB::table('mentoria_notebooks')
        ...
        DB::table('mentoria_notebooks')->where('id', $notebook)->update($data);
    ...
    {
        $updated = DB::table('mentoria_notebooks')
    ...
    {
        return DB::table('mentoria_scheduled_reviews')
            ->where('tenant_id', $tenant)
            ->where('student_id', $studentId)
    ...
    {
        DB::table('mentoria_mock_exam_subject_results')->where('tenant_id', $tenant)->where('mock_exam_id', $mockId)->delete();
        ...
        foreach ($results as $result) {
            ...
            if (! empty($result['subject_id'])) {
                $this->requireTenantRow('mentoria_subjects', $tenant, $result['subject_id']);
            }

            DB::table('mentoria_mock_exam_subject_results')->insert([
                'id' => (string) Str::uuid(),
                'tenant_id' => $tenant,
                'mock_exam_id' => $mockId,
    ...
    private function assertReviewContent(int $tenant, int $studentId, array $data): void
    {
        ...
        $subject = DB::table('mentoria_subjects')
    ...
    {
        $aliases = [
            'contest_id' => ['mentoria_contests', 'contest_id'],
            'edict_id' => ['mentoria_edicts', 'edict_id'],
            'subject_id' => ['mentoria_subjects', 'subject_id'],
            'topic_id' => ['mentoria_topics', 'topic_id'],
            'subtopic_id' => ['mentoria_subtopics', 'subtopic_id'],
            'materiaId' => ['mentoria_subjects', 'subject_id'],
            'topicoId' => ['mentoria_topics', 'topic_id'],
            'subtopicoId' => ['mentoria_subtopics', 'subtopic_id'],
        ];

        foreach ($aliases as $field => [$table]) {
            if (! empty($data[$field]) && ! DB::table($table)->where('tenant_id', $tenant)->where('id', $data[$field])->exists()) {
                throw new NotFoundHttpException('Referência Mentoria não encontrada neste tenant.');
            }
        }
    }
    ...
    {
        if (! DB::table('mentoria_student_contests')
            ->where('tenant_id', $tenant)
            ->where('student_id', $studentId)
            ->where('contest_id', $contestId)
            ->where('is_active', true)
    ...
    {
        return DB::table('mentoria_schedules')
            ->where('tenant_id', $tenant)
            ->where('student_id', $studentId)
            ->where('is_active', true)
    ...
    {
        $row = DB::table('mentoria_schedules')
            ->where('tenant_id', $tenant)
            ->where('student_id', $studentId)
            ->where('id', $schedule)
            ->where('is_active', true)
    ...
    {
        $row = DB::table('mentoria_schedule_items as i')
            ->join('mentoria_schedules as s', 's.id', '=', 'i.schedule_id')
            ->where('i.tenant_id', $tenant)
