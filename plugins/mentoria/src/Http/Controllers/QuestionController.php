...
class QuestionController extends Controller
{
    ...
    public function update(Request $request, string $question): JsonResponse
    {
        ...
        $data['updated_at'] = now();
        DB::table('mentoria_question_bank')->where('tenant_id', $tenantId)->where('id', $question)->update($data);
    ...
    public function studentHistory(Request $request, int $tenant, string $question): JsonResponse
    {
        ...
        $history = DB::table('mentoria_question_answers as a')
            ->join('mentoria_question_bank as q', 'q.id', '=', 'a.question_id')
    ...
    public function studentStatistics(Request $request, int $tenant): JsonResponse
    {
        ...
        $base = DB::table('mentoria_question_answers as a')
            ->join('mentoria_question_bank as q', 'q.id', '=', 'a.question_id')
            ->where('a.tenant_id', $tenant)
            ->where('a.student_id', $student->id)
    ...
    private function insertQuestion(int $tenantId, int $actorId, array $data): string
    {
        ...
        $id = (string) Str::uuid();
        DB::table('mentoria_question_bank')->insert([
            'id' => $id,
            'tenant_id' => $tenantId,
    ...
    {
        $query = DB::table('mentoria_question_bank')->where('tenant_id', $tenantId)->where('id', $questionId);
        if ($activeOnly) {
    ...
    {
        if (! DB::table('mentoria_contests')->where('tenant_id', $tenantId)->where('id', $contestId)->where('is_active', true)->exists()) {
    ...
    {
        return DB::table('mentoria_student_contests')
            ->where('tenant_id', $tenantId)
    ...
    private function resolveSubjectId(int $tenantId, int $studentId, ?string $contestId, string $subjectName): ?string
    {
        ...
        return DB::table('mentoria_subjects as s')
            ...
            ->join('mentoria_student_edicts as se', 'se.edict_id', '=', 'e.id')
