...
{
    /**
     * Port of the useful scheduling behavior from the original CJC:
    ...
    public function reprogram(int $tenantId, int $studentId, string $scheduleId, ?string $fromDate = null): int
    {
        ...
        $items = DB::table('cjc_schedule_items')
            ->where('tenant_id', $tenantId)
    ...
    private function diagnostic(int $studentId, int $tenantId, string $contestId, array $configuration): array
    {
        ...
        $subjectsRows = DB::table('cjc_subjects')
            ->where('tenant_id', $tenantId)
        ...
        foreach ($subjectsRows as $subject) {
            ...
            $questionStats = DB::table('cjc_question_logs')
                ->where('tenant_id', $tenantId)
                ->where('student_id', $studentId)
    ...
    private function fixedOccupancy(int $tenantId, int $studentId, string $scheduleId, CarbonImmutable $start): array
    {
        ...
        $fixedItems = DB::table('cjc_schedule_items')
            ->where('tenant_id', $tenantId)
            ->where('schedule_id', $scheduleId)
        ...
        $reviews = DB::table('cjc_scheduled_reviews')
            ...
            ->where('next_date', '>=', $start->toDateString())
            ->get(['next_date']);

        foreach ($reviews as $review) {
            $key = (string) $review->next_date;
            $occupied[$key] ??= ['minutes' => 0, 'count' => 0];
            $occupied[$key]['minutes'] += 30;
            $occupied[$key]['count']++;
        }
    ...
    {
        $ok = DB::table('cjc_student_contests')
            ->where('tenant_id', $tenantId)
            ->where('student_id', $studentId)
            ->where('contest_id', $contestId)
            ->where('is_active', true)
