...
class ContentController extends Controller
{
    ...
    public function updateContest(Request $request, string $contest): JsonResponse
    {
        ...
        $data['updated_at'] = now();
        DB::table('cjc_contests')->where('tenant_id', $tenantId)->where('id', $contest)->update($data);
    ...
    public function destroyContest(Request $request, string $contest): JsonResponse
    {
        ...
        DB::transaction(function () use ($tenantId, $contest): void {
            DB::table('cjc_contests')->where('tenant_id', $tenantId)->where('id', $contest)
                ->update(['is_active' => false, 'updated_at' => now()]);
            DB::table('cjc_student_contests')->where('tenant_id', $tenantId)->where('contest_id', $contest)
                ->update(['is_active' => false, 'updated_at' => now()]);
            DB::table('cjc_edicts')->where('tenant_id', $tenantId)->where('contest_id', $contest)
                ->update(['is_active' => false, 'updated_at' => now()]);
        });
    ...
    public function updateStudentContest(Request $request, string $contest, int $student): JsonResponse
    {
        ...
        $tenantId = $this->access->tenantId($actor);
        $studentUser = $this->access->assertStudentInTenant($actor, $student);
        $row = DB::table('cjc_student_contests')
        ...
        $data['updated_at'] = now();
        DB::table('cjc_student_contests')->where('id', $row->id)->update($data);
    ...
    public function unassignContest(Request $request, string $contest, int $student): JsonResponse
    {
        ...
        $tenantId = $this->access->tenantId($actor);
        $studentUser = $this->access->assertStudentInTenant($actor, $student);
        $updated = DB::table('cjc_student_contests')
    ...
    public function updateEdict(Request $request, string $edict): JsonResponse
    {
        ...
        $data['updated_at'] = now();
        DB::table('cjc_edicts')->where('tenant_id', $tenantId)->where('id', $edict)->update($data);
    ...
    public function destroyEdict(Request $request, string $edict): JsonResponse
    {
        ...
        DB::transaction(function () use ($tenantId, $edict): void {
            $subjectIds = DB::table('cjc_subjects')->where('tenant_id', $tenantId)->where('edict_id', $edict)->pluck('id');
            $topicIds = DB::table('cjc_topics')->where('tenant_id', $tenantId)->whereIn('subject_id', $subjectIds)->pluck('id');

            DB::table('cjc_subtopics')->where('tenant_id', $tenantId)->whereIn('topic_id', $topicIds)->update(['is_active' => false, 'updated_at' => now()]);
            DB::table('cjc_topics')->where('tenant_id', $tenantId)->whereIn('subject_id', $subjectIds)->update(['is_active' => false, 'updated_at' => now()]);
            DB::table('cjc_subjects')->where('tenant_id', $tenantId)->where('edict_id', $edict)->update(['is_active' => false, 'updated_at' => now()]);
            DB::table('cjc_edicts')->where('tenant_id', $tenantId)->where('id', $edict)->update(['is_active' => false, 'updated_at' => now()]);
            DB::table('cjc_student_edicts')->where('tenant_id', $tenantId)->where('edict_id', $edict)->update(['is_active' => false, 'updated_at' => now()]);
        });
    ...
    {
        return $this->updateHierarchyItem($request, 'cjc_subjects', $subject);
    }
    ...
    public function destroySubject(Request $request, string $subject): JsonResponse
    {
        $actor = $request->user();
        $tenantId = $this->access->tenantId($actor);
        $this->requireRow('cjc_subjects', $subject, $tenantId);
        $topicIds = DB::table('cjc_topics')->where('tenant_id', $tenantId)->where('subject_id', $subject)->pluck('id');

        DB::transaction(function () use ($tenantId, $subject, $topicIds): void {
            DB::table('cjc_subtopics')->where('tenant_id', $tenantId)->whereIn('topic_id', $topicIds)->update(['is_active' => false, 'updated_at' => now()]);
            DB::table('cjc_topics')->where('tenant_id', $tenantId)->where('subject_id', $subject)->update(['is_active' => false, 'updated_at' => now()]);
            DB::table('cjc_subjects')->where('tenant_id', $tenantId)->where('id', $subject)->update(['is_active' => false, 'updated_at' => now()]);
        });
    ...
    {
        return $this->updateHierarchyItem($request, 'cjc_topics', $topic, 500);
    }
    ...
    public function destroyTopic(Request $request, string $topic): JsonResponse
    {
        $actor = $request->user();
        $tenantId = $this->access->tenantId($actor);
        $this->requireRow('cjc_topics', $topic, $tenantId);

        DB::transaction(function () use ($tenantId, $topic): void {
            DB::table('cjc_subtopics')->where('tenant_id', $tenantId)->where('topic_id', $topic)->update(['is_active' => false, 'updated_at' => now()]);
            DB::table('cjc_topics')->where('tenant_id', $tenantId)->where('id', $topic)->update(['is_active' => false, 'updated_at' => now()]);
        });
    ...
    {
        return $this->updateHierarchyItem($request, 'cjc_subtopics', $subtopic, 500);
    }
    ...
    public function destroySubtopic(Request $request, string $subtopic): JsonResponse
    {
        $actor = $request->user();
        $tenantId = $this->access->tenantId($actor);
        $this->requireRow('cjc_subtopics', $subtopic, $tenantId);
        DB::table('cjc_subtopics')->where('tenant_id', $tenantId)->where('id', $subtopic)
            ->update(['is_active' => false, 'updated_at' => now()]);
        ...
        DB::transaction(function () use ($tenantId, $edict, $data): void {
            foreach ($data['items'] as $item) {
                $table = match ($item['type']) {
                    'subject' => 'cjc_subjects',
                    'topic' => 'cjc_topics',
                    'subtopic' => 'cjc_subtopics',
                };
    ...
    public function bulkAssignEdict(Request $request, string $edict): JsonResponse
    {
        ...
        $actor = $request->user();
        $tenantId = $this->access->tenantId($actor);
        $edictRow = $this->requireRow('cjc_edicts', $edict, $tenantId);
        $students = $this->studentsForBulkAudience($tenantId, $data);
        $count = 0;
    ...
    public function unassignEdict(Request $request, string $edict, int $student): JsonResponse
    {
        ...
        $tenantId = $this->access->tenantId($actor);
        $studentUser = $this->access->assertStudentInTenant($actor, $student);
        $updated = DB::table('cjc_student_edicts')
    ...
    {
        $existing = DB::table('cjc_student_contests')
        ...
        $id = (string) Str::uuid();
        DB::table('cjc_student_contests')->insert($payload + [
            'id' => $id,
            'tenant_id' => $tenantId,
    ...
    {
        $existing = DB::table('cjc_student_edicts')
            ->where('tenant_id', $tenantId)->where('student_id', $studentId)->where('edict_id', $edictId)->first();

        if ($existing) {
            DB::table('cjc_student_edicts')->where('id', $existing->id)->update([
                'assigned_by' => $actorId,
                'is_active' => true,
                'updated_at' => now(),
            ]);
