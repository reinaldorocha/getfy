...
class StudentController extends Controller
{
    ...
    public function updateEdictProgress(Request $request, int $tenant, string $item): JsonResponse
    {
        ...
        $existing = DB::table('cjc_edict_progress')
        ...
        DB::transaction(function () use ($tenant, $student, $type, $item, $existing, $payload, $data, $context): void {
            if ($existing) {
                ...
                DB::table('cjc_edict_progress')->insert($payload + [
                    'id' => (string) Str::uuid(),
                    'tenant_id' => $tenant,
            ...
            if (! $data['studied']) {
                $query = DB::table('cjc_scheduled_reviews')
                    ->where('tenant_id', $tenant)
                    ->where('student_id', $student->id)
            ...
            foreach ($intervals as $index => $days) {
                ...
                if ($review) {
                    DB::table('cjc_scheduled_reviews')->where('id', $review->id)->update($reviewPayload);
                } else {
                    DB::table('cjc_scheduled_reviews')->insert($reviewPayload + [
                        'id' => (string) Str::uuid(),
                        'tenant_id' => $tenant,
                        'student_id' => $student->id,
        ...
        $reprogrammed = 0;
        $warning = null;
        $schedule = DB::table('cjc_schedules')
            ->where('tenant_id', $tenant)
            ->where('student_id', $student->id)
            ->where('contest_id', $context->contest_id)
            ->where('is_active', true)
    ...
    private function payload(User $student, int $tenantId, ?string $requestedContestId = null): array
    {
        ...
        $progress = DB::table('cjc_edict_progress')
            ->where('tenant_id', $tenantId)
            ->where('student_id', $studentId)
            ->get();

        $materials = $this->visibleMaterials($student, $tenantId);
        $materialProgress = DB::table('cjc_material_progress')
            ->where('tenant_id', $tenantId)
            ->where('student_id', $studentId)
            ->get()
            ->keyBy('material_id');
        ...
        $materials = $materials->map(function ($material) use ($materialProgress, $tenantId) {
            ...
            $material->download_url = $material->type === 'arquivo'
                ? url('/cjc-estudos/'.$tenantId.'/materials/'.$material->id.'/download')
                : null;
            return $material;
        });
        ...
        $scheduleItems = DB::table('cjc_schedule_items')
            ->where('tenant_id', $tenantId)
        ...
        $lastAnswers = DB::table('cjc_question_answers')
            ->where('tenant_id', $tenantId)
            ->where('student_id', $studentId)
        ...
        $mockSubjectResults = DB::table('cjc_mock_exam_subject_results')
            ->where('tenant_id', $tenantId)
            ->whereIn('mock_exam_id', $mockIds)
            ->where('is_active', true)
            ->get();

        $examConfigurations = DB::table('cjc_exam_configurations')
            ->where('tenant_id', $tenantId)
            ->where('student_id', $studentId)
            ->get()
            ->map(function ($row) {
        ...
        $questionLogs = DB::table('cjc_question_logs')
            ->where('tenant_id', $tenantId)
            ->where('student_id', $studentId)
            ->where('is_active', true)
            ->latest('recorded_at')
    ...
    private function visibleMaterials(User $student, int $tenantId)
    {
        ...
        return DB::table('cjc_support_materials')
            ->where('tenant_id', $tenantId)
            ->whereIn('id', $ids)
            ->where('is_active', true)
    ...
    {
        if ($type === 'topico') {
            $row = DB::table('cjc_topics as t')
                ->join('cjc_subjects as s', 's.id', '=', 't.subject_id')
                ->join('cjc_edicts as e', 'e.id', '=', 's.edict_id')
                ->join('cjc_contests as c', 'c.id', '=', 'e.contest_id')
                ->join('cjc_student_edicts as se', function ($join) use ($studentId): void {
            ...
            $row = DB::table('cjc_subtopics as st')
                ->join('cjc_topics as t', 't.id', '=', 'st.topic_id')
                ->join('cjc_subjects as s', 's.id', '=', 't.subject_id')
                ->join('cjc_edicts as e', 'e.id', '=', 's.edict_id')
                ->join('cjc_contests as c', 'c.id', '=', 'e.contest_id')
                ->join('cjc_student_edicts as se', function ($join) use ($studentId): void {
