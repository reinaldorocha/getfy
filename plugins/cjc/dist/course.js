...
export const CjcCourseBuilder = {
    name: 'CjcCourseBuilder',
    ...
    setup(props) {
        ...
        async function load() {
            ...
            message.value = '';
            try {
                const data = await api('/cjc/course-products/' + props.produto.id + '/questions');
        ...
        async function save() {
            ...
            try {
                ...
                await api('/cjc/course-products/' + props.produto.id + '/lessons/' + selectedLesson.value + '/questions', {
                    method: 'PUT',
        ...
        return () => h('div', { class: 'space-y-4 p-5' }, [
            card([
                sectionTitle('Exercícios CJC', 'Escolha uma aula e vincule questões já existentes no Banco de Questões do CJC.'),
...
export const CjcLessonExercises = {
    name: 'CjcLessonExercises',
    props: {
    ...
    setup(props) {
        ...
        async function load() {
            try {
                const data = await api('/cjc-course/products/' + props.product.id + '/lessons/' + props.lesson.id + '/questions');
                questions.value = data.questions || [];
            } catch (e) {
                message.value = e.message;
            } finally {
        ...
        async function answer(q) {
            ...
            try {
                const result = await api('/cjc-course/products/' + props.product.id + '/lessons/' + props.lesson.id + '/questions/' + q.id + '/answer', {
                    method: 'POST',
        ...
        return () => {
            ...
            return h('section', { class: 'rounded-2xl border border-zinc-800 bg-zinc-950/60 p-5' }, [
