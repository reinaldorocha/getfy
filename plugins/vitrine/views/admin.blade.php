...
<body class="bg-[#070707] text-[#e2e2e2] min-h-screen flex flex-col">
        ...
        <section id="tab-approvals" class="tab-pane hidden space-y-6">
            <div class="bg-[#121212] border border-[#1c1c1c] p-6 rounded-2xl space-y-6">
                <div>
                    ...
                    <p class="text-xs text-white/50 mt-1">Gerencie as fotos e configure a rolagem automática dos depoimentos na vitrine.</p>
                </div>

                <!-- Autoplay & Speed Configuration Card -->
                <div class="p-5 bg-[#181818] rounded-xl border border-white/5 space-y-4">
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div>
                            <h4 class="text-xs font-black uppercase tracking-wider text-brand-light-magenta flex items-center gap-2">
                                <i data-lucide="play-circle" class="w-4 h-4 text-brand-magenta"></i>
                                Rolagem Automática do Carrossel (Autoplay)
                            </h4>
                            <p class="text-[11px] text-white/50 mt-0.5">
                                Quando ativado, os depoimentos passam automaticamente sem o cliente precisar clicar nas setas.
                            </p>
                        </div>

                        <div class="flex items-center gap-3">
                            <span class="text-xs text-white/70 font-semibold">Passar Automaticamente</span>
                            <label class="relative inline-flex items-center cursor-pointer">
                                <input type="checkbox" id="cfg-approvals_autoplay" class="sr-only peer" {{ ($settings->approvals_autoplay ?? true) ? 'checked' : '' }}>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-3 border-t border-white/5 items-end">
                        <div>
                            <label class="block text-[11px] font-bold uppercase text-white/70 mb-1">
                                Velocidade de Transição (em segundos)
                            </label>
                            <div class="flex items-center gap-2">
                                <input 
                                    type="number" 
                                    id="cfg-approvals_speed" 
                                    value="{{ (int) ($settings->approvals_speed ?? 4) }}" 
                                    min="1" 
                                    max="30" 
                                    step="1" 
                                    class="w-full bg-[#1c1c1c] border border-[#2d2d2d] focus:border-brand-magenta rounded-lg px-3.5 py-2 text-xs text-white outline-none" 
                                />
                                <span class="text-xs text-white/50 shrink-0 font-medium">segundos</span>
                            </div>
                            <p class="text-[10px] text-white/40 mt-1">Tempo de espera em cada depoimento (Padrão: 4 segundos).</p>
                        </div>

                        <div class="flex justify-end">
                            <button type="button" onclick="saveApprovalsSettings()" class="btn-magenta px-5 py-2.5 rounded-lg text-xs font-black uppercase tracking-wider cursor-pointer shadow-lg flex items-center gap-2">
                                <i data-lucide="check" class="w-4 h-4"></i> Salvar Configurações do Carrossel
                            </button>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        async function saveApprovalsSettings() {
            const approvals_autoplay = document.getElementById('cfg-approvals_autoplay').checked ? 1 : 0;
            const approvals_speed = Math.max(1, parseInt(document.getElementById('cfg-approvals_speed').value, 10) || 4);

            try {
                const res = await fetch('{{ url("/vitrine/admin/settings") }}', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': '{{ csrf_token() }}' },
                    body: JSON.stringify({ approvals_autoplay, approvals_speed })
                });
                if (res.ok) {
                    showToast('Configurações do carrossel salvas com sucesso!');
                } else {
        ...
        async function saveAllSettings() {
            const settings = {
                ...
                approvals_autoplay: document.getElementById('cfg-approvals_autoplay')?.checked ? 1 : 0,
                approvals_speed: Math.max(1, parseInt(document.getElementById('cfg-approvals_speed')?.value, 10) || 4),
            };
